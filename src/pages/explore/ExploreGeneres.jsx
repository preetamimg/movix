...
const Explore = () => {
    ...
    let generId = filterGener?.[0]?.id;

    
    const fetchInitialData = () => {
        setLoading(true);
        console.log('genre id', generId)
        setTimeout(() => {
            FetchDataFromApi(`/discover/movie?with_genres=${generId}`, filters).then((res) => {
                console.log('res', res)
                setData(res);
    ...
    FetchDataFromApi(
            `/discover/movie?page=${pageNum}&with_genres=${generId}`,
            filters
        ).then((res) => {
            if (data?.results) {
                setData({
